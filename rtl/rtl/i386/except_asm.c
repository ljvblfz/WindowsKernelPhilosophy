...
//CaptureRest:
_declspec(naked) CaptureRest(){ _asm{
...
_declspec(naked) RtlpExecuteHandler(){ _asm{
    /* Save non-volatile */
    push ebx
    push esi
    push edi
...
_declspec(naked) RtlpExecuteHandler2(){ _asm{
    /* Set up stack frame */
    push ebp
    mov ebp, esp
...
_declspec(naked) RtlpExceptionProtector(){ _asm{
...
_declspec(naked) RtlpUnwindProtector(){ _asm{
    /* Assume we'll continue */
    mov eax, ExceptionContinueSearch
