...
DECLSPEC_NORETURN VOID _declspec(naked) FASTCALL KiEditedTrapReturn(IN PKTRAP_FRAME TrapFrame){ _asm{
/*
	mov     esp, [ecx+14h]
...
DECLSPEC_NORETURN VOID _declspec(naked) FASTCALL KiTrapReturnNoSegments(IN PKTRAP_FRAME TrapFrame){ _asm {
/*
	lea     esp, [ecx+68h]
	mov     ebx, [ecx+5Ch]
	mov     esi, [ecx+58h]
...
unsigned long _declspec(naked) ___readcr4(void){ _asm{
	//mov eax, cr4
	_emit 0x0F 
	_emit 0x20 
	_emit 0xE0 
...
VOID _declspec(naked) NTAPI ExpInterlockedPopEntrySListFault(VOID){ _asm{
	mov     ebx, [eax]
...
VOID _declspec(naked) NTAPI ExpInterlockedPopEntrySListResume(VOID){ _asm{
ExpResume:
		mov     edx, [ebp+4]
		mov     eax, [ebp+0]
		or      eax, eax
...
VOID _declspec(naked) _cdecl KiInterruptTemplate(VOID){ _asm{
		sub     esp, 68h
		mov     [esp+96], ebp
		mov     [esp+92], ebx
		mov     [esp+88], esi
...
_declspec(naked) KiInterruptTemplateObject(VOID){ _asm {
...
__declspec(naked) 
FASTCALL 
__InterlockedIncrement(PLONG Addend){ _asm{
	mov     eax, 1
	lock xadd [ecx], eax
	inc     eax
	retn
...
__declspec(naked) 
FASTCALL 
__InterlockedDecrement(PLONG Addend){ _asm{
	or      eax, 0FFFFFFFFh
	lock xadd [ecx], eax
	dec     eax
	retn
