...
_declspec(naked) _KiExceptionExit2(){ _asm{
...
_declspec(naked) _KiExceptionExit(){ _asm{
...
_declspec(naked) CommonDispatchException2(){ _asm{
...
_declspec(naked) CommonDispatchException(){ _asm{
...
_declspec(naked) _KiTrap00(){ _asm{ 
		push    0	 // 压入伪错误代码
		ENTER_TRAP2 
...
_declspec(naked) _KiTrap01(){ _asm{ 
		push    0	// 压入伪错误代码
		ENTER_TRAP 
...
_declspec(naked) _KiTrap03(){ _asm{
		push    0  // 压入伪错误代码
        ENTER_TRAP
...
_declspec(naked) _KiTrap0D(){ _asm{
		//push    0		// 不需要压入伪错误代码
        ENTER_TRAP2
...
_declspec(naked) _KiTrap0E(){ _asm{
		//push    0		// 不需要压入伪错误代码
		ENTER_TRAP2
...
_declspec(naked) _KiDebugService(){ _asm{
		push    0	// 压入伪错误代码
        ENTER_TRAP2
...
VOID _declspec(naked) NTAPI KiStartUnexpectedRange(VOID){ _asm{
		push    0xFFFFFFB0
...
VOID _declspec(naked) NTAPI KiUnexpectedInterruptTail(VOID){ _asm{
		sub     esp, 64h 
		mov     [esp+60h], ebp
		mov     [esp+5Ch], ebx
		mov     [esp+58h], esi
...
VOID _declspec(naked) NTAPI KiEndUnexpectedRange(VOID){ _asm{
