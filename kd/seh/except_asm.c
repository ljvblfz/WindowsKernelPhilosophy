...
_declspec(naked) RtlpGetExceptionList(){ _asm{
...
_declspec(naked) RtlpSetExceptionList(){ _asm{
    /* Get the new list */
    mov ecx, [esp+4]
    mov ecx, [ecx]
...
VOID _declspec(naked) NTAPI RtlCaptureContext(_Out_ PCONTEXT ContextRecord){ _asm{
...
_declspec(naked) RtlpCaptureContext(){ _asm{
    /* Preserve EBX and put the context in it */
    push ebx
    mov ebx, [esp+8]
...
//CaptureRest:
_declspec(naked) NTAPI CaptureRest(){ _asm{
...
_declspec(naked) RtlpExecuteHandlerForException(){ _asm{
...
_declspec(naked) RtlpExecuteHandlerForUnwind(){ _asm{ 
    /* Copy the routine in EDX */
...
_declspec(naked) NTAPI RtlpExecuteHandler(){ _asm{
    /* Save non-volatile */
    push ebx
    push esi
    push edi
...
_declspec(naked) NTAPI RtlpExecuteHandler2(){ _asm{
    /* Set up stack frame */
    push ebp
    mov ebp, esp
...
_declspec(naked) NTAPI RtlpExceptionProtector(){ _asm{
...
_declspec(naked) NTAPI RtlpUnwindProtector(){ _asm{
    /* Assume we'll continue */
    mov eax, ExceptionContinueSearch
...
VOID _declspec(naked) NTAPI RtlRaiseException(_In_ PEXCEPTION_RECORD ExceptionRecord){ _asm{
    /* Set up stack frame */
    push ebp
    mov ebp, esp
...
//_RtlRaiseStatus@4:
VOID _declspec(naked) NTAPI RtlRaiseStatus(_In_ NTSTATUS Status){ _asm{
    /* Set up stack frame */
    push ebp
    mov ebp, esp
